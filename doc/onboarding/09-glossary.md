...
| **Yield/resume (NEP-519)** | NEAR-runtime primitive that lets a contract **park a Promise on-chain** (via `env::promise_yield_create`) and have a later tx resume it (via `env::promise_yield_resume`). It is *not* an SDK abstraction — the parking happens in the NEAR VM. Enables `sign()` to look synchronous from the caller's perspective even though the MPC network does the work asynchronously. See [§4.2](04-event-flow.md#nears-on-chain-yieldresume-primitive-nep-519). |
