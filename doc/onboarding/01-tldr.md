...
| **Deposit-based anti-spam** | Attach a fee to `sign()`. Gates abuse. Dynamic on NEAR (scales with load, [contract/src/lib.rs:231-242](../../chain-signatures/contract/src/lib.rs#L231-L242)), fixed on Ethereum. On Solana the field exists in `ProgramState` and is echoed into the event, but **no transfer is enforced** — see [§4.3](04-event-flow.md#checkpoint-1--contract-side) and [§11](11-funds-and-refunds.md). | NEAR (dynamic), Ethereum (fixed); Solana non-functional |
